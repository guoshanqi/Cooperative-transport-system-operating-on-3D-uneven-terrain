...
while sim.getSimulationTime() < this_sim.max_simtime:
    ...
    if controller.trj_x.all() and controller.trj_y.all():
        ...
        if index:
            ...
            vc1 = np.clip(vc1/1.29, controller.u_min[0], 0.2)
            wc1 = np.clip(wc1, controller.u_min[1], controller.u_max[1])
            vc2 = np.clip(vc2/1.29, controller.u_min[0], 0.2)
            wc2 = np.clip(wc2, controller.u_min[1], controller.u_max[1])
            vc3 = np.clip(vc3/1.29, controller.u_min[0], 0.2)
            wc3 = np.clip(wc3, controller.u_min[1], controller.u_max[1])
            vc4 = np.clip(vc4/1.29, controller.u_min[0], 0.2)
            wc4 = np.clip(wc4, controller.u_min[1], controller.u_max[1])
            ...
            this_sim.set_shaft_force(1000, 1000, 1000, 1000)
            
            # 将数据添加到列表中
            data.append([sim_time, payload_pos[0], payload_pos[1], payload_pos[2], #
...
df.to_csv('./data/ref_data_random.csv', index=False)

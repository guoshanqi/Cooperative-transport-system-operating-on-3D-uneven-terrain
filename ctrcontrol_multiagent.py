...
if __name__ == "__main__":
    ...
    # ref_trj_circle = control_ref.generate_ref_trj_circle()
    # sim.createPath(ref_trj_circle, 8, len(ref_trj_circle), 1.0, 0, [0.0, 0.0, 1.0])  # Create a path for the simulation (optional)
    ...
    while sim.getSimulationTime() < this_sim.max_simtime:
        ...
        if sub_controller.trj_x.all() and sub_controller.trj_y.all():
            ...
            a11, a12, a13, a14= K1 @ err1, K1 @ err_12, K1 @ err_13, K1 @ err_14
            net_fd1 = np.linalg.pinv(M_c1) @ output1.numpy() * 1e-4
            u1 = sub_controller.u_ref + a11 + a12  + net_fd1
            ...
            a21, a22, a23, a24 = K2 @ err2, K2 @ err21, K2 @ err23, K2 @ err24
            net_fd2 = np.linalg.pinv(M_c2) @ output2.numpy() * 1e-4 
            u2 = sub_controller.u_ref + a21 + a22 + a24 + net_fd2
            ...
            a31, a32, a33, a34 = K3 @ err3, K3 @ err31, K3 @ err32, K3 @ err34
            net_fd3 = np.linalg.pinv(M_c3) @ output3.numpy() * 1e-4
            u3 = sub_controller.u_ref + a31  + a34 + net_fd3
            ...
            a41, a42, a43, a44 = K4 @ err4, K4 @ err_14, K4 @ err_24, K4 @ err_34
            net_fd4 = np.linalg.pinv(M_c4) @ output4.numpy() * 1e-4
            u4 = sub_controller.u_ref + a41 + a43 + a44 + net_fd4
    ...
    filename = f'./new_data/H_data_multi_sce1_{current_time}.csv'
